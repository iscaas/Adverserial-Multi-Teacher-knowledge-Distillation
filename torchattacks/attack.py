...
class Attack(object):
    ...
    def get_logits(self, inputs, labels=None, *args, **kwargs):
        #convert input images from torch to tensorflow
        import tensorflow as tf
        # numpy_array = inputs.detach().cpu().numpy()
        # input = tf.convert_to_tensor(numpy_array) # Move to device
        
        #code for getting logits
        if self._normalization_applied:
            inputs = self.normalize(inputs)   
        # logits = self.model(inputs)
        
        with tf.device('/gpu:0'): # Move to device
            logits = self.model(inputs)
        
        
        #convert output logits from tensorflow to torch
        # numpy_array = logits.numpy()
        # logits = torch.from_numpy(numpy_array).to(self.device)
