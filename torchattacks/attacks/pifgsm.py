...
class PIFGSM(Attack):
    ...
    def forward(self, images, labels):
        ...
        images_min = self.clip_by_tensor(
            images-self.max_epsilon, t_min=0, t_max=1)
        images_max = self.clip_by_tensor(
            images+self.max_epsilon, t_min=0, t_max=1)
    ...
    def project_noise(self, images, P_kern, padding_size):
        images = F.conv2d(images, P_kern, padding=(
    ...
    def graph(self, images, labels, images_min, images_max):
        ...
        for _ in range(num_iter):
            ...
            cut_noise = torch.clamp(
                abs(amplification) - eps, 0, 10000.0) * torch.sign(amplification)
            projection = gamma * \
